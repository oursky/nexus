...
// Grant holds a minted auth relay grant.
type Grant struct {
	WorkspaceID string
...
// Broker mints and consumes short-lived auth relay tokens.
type Broker struct {
	mu     sync.Mutex
...
// Mint creates a new token for the given workspace and env payload.
...
// Consume validates and consumes a token, returning its env payload.
...
// Revoke explicitly invalidates a token.
