// Package ptyhost implements the PTY host RPC server used by the daemon.
//
// The same binary runs as a long-lived subprocess when invoked as
// "nexus __pty-host --socket=..." (see [Main]).
package ptyhost
...
// HiddenSubcommand is the argv[1] token that runs the PTY host inside the nexus binary.
const HiddenSubcommand = "__pty-host"

var ErrSessionNotFound = fmt.Errorf("session not found")

// Main runs the PTY host process. It expects argv like the standalone pty-host:
//
//	nexus __pty-host --socket=/path/to/sock
//
// Call from cmd/nexus after stripping the __pty-host token from os.Args.
func Main() {
