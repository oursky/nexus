...
func installVMRootfsRootless(w io.Writer, emitJSON bool) error {
	...
	lockPath := dest + ".lock"
	return withInstallLock(lockPath, func() error {
		if _, err := os.Stat(dest); err == nil {
			return nil
		}
...
func withInstallLock(lockPath string, fn func() error) error {
	...
	if err := syscall.Flock(int(lockFile.Fd()), syscall.LOCK_EX); err != nil {
		return fmt.Errorf("lock install file: %w", err)
	}
	defer func() {
