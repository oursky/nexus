...
public final class HeadlessRPCServer {
    ...
    private func handleDaemonProvision(body: String) async -> (Int, String) {
        ...
        do {
            ...
            let payload: [String: Any] = ["ok": true, "lastPhase": lastPhase]
            guard let data = try? JSONSerialization.data(withJSONObject: payload),
                  let str = String(data: data, encoding: .utf8) else {
                return (500, jsonError("serialization failed"))
            }
    ...
    private func handleLinuxboxCleanRoom(body: String) async -> (Int, String) {
        ...
        let cleanRoomScript = """
            trap '' PIPE
            set -uo pipefail
            echo "==> Stopping nexus daemon if running..."
            nohup bash -c 'sleep 0.2; pkill -x nexus 2>/dev/null; true' </dev/null >/dev/null 2>&1 &
